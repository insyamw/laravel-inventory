...
class RestorationController extends Controller
{
    public function store(Request $request, $lending_id)
    {
        try {
            ...
            $totalStuffRestoration = (int)$request->total_good_stuff + (int)$request->total_defac_stuff;
            if ((int) $totalStuffRestoration > (int) $lending['total_stuff']) {
                ...
                $totalDefecStock = (int)$stuffStock['total_defec'] + (int)$request->total_defac_stuff;
                $stuffStock->update([
